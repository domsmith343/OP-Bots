...
async def test_news_command(mock_context):
    from nami.commands.news import NewsCommand
    
    with patch('nami.commands.news.NewsAPI') as mock_news:
...
async def test_weather_command(mock_context):
    from nami.commands.weather import WeatherCommand
    
    with patch('nami.commands.weather.WeatherAPI') as mock_weather:
...
async def test_crypto_command(mock_context):
    from nami.commands.crypto import CryptoCommand
    
    with patch('nami.commands.crypto.CryptoAPI') as mock_crypto:
...
async def test_dailybrief_command(mock_context):
    from nami.commands.dailybrief import DailyBriefCommand
    
    with patch('nami.commands.dailybrief.NewsAPI') as mock_news, \
         patch('nami.commands.dailybrief.WeatherAPI') as mock_weather, \
         patch('nami.commands.dailybrief.CryptoAPI') as mock_crypto:
